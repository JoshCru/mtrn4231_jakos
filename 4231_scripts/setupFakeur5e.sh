...
gnome-terminal -t "DriverServer" -- bash -c 'export FASTRTPS_DEFAULT_PROFILES_FILE=/tmp/fastdds_profile.xml; export RMW_IMPLEMENTATION=rmw_fastrtps_cpp; source ../ros2_system/install/setup.bash; ros2 launch ur_robot_driver ur_control.launch.py ur_type:=ur5e robot_ip:=yyy.yyy.yyy.yyy initial_joint_controller:=joint_trajectory_controller use_fake_hardware:=true launch_rviz:=false description_file:=ur5e_with_end_effector.urdf.xacro description_package:=motion_control_module; exec bash'

sleep 5

gnome-terminal -t "MoveitServer" -- bash -c 'export FASTRTPS_DEFAULT_PROFILES_FILE=/tmp/fastdds_profile.xml; export RMW_IMPLEMENTATION=rmw_fastrtps_cpp; source ../ros2_system/install/setup.bash; ros2 launch motion_control_module ur5e_moveit_with_gripper.launch.py ur_type:=ur5e launch_rviz:=true use_fake_hardware:=true; exec bash'
