...
Safety Boundaries (relative to base_link):
  - x >= -600mm, y >= -300mm, 0mm <= z <= 655mm

Features:
  - Orientation constraint: End-effector always faces down (rx=3.14, ry=0.001, rz=0.001)
  - Home position: Calibrated from joint angles [0, -75, 90, -105, -90] degrees
  - Smooth paths: Configurable max_step to avoid extravagant maneuvers
...
Usage:
    ...
    # Move with constrained orientation (simple)
    robot.move(x=100, y=400, z=300)

    # Move with custom orientation (advanced)
    robot.move_to_cartesian(x=100, y=400, z=300, rx=3.14, ry=0.001, rz=0.001)
"""
...
# Adjusted to include home position
SAFETY_BOUNDS = {
    'x_min': -0.6,   # Adjusted from -0.3 to include home at -588.63mm
...
# Default orientation constraint: End-effector facing down
# rx ≈ π (3.14), ry ≈ 0, rz ≈ 0
DEFAULT_ORIENTATION = {
    'rx': 3.14,    # π radians (180° around x-axis)
    'ry': 0.001,
    'rz': 0.001,
}

# Home position (calibrated from joint angles [0, -75, 90, -105, -90] degrees)
HOME_JOINT_ANGLES = [0.0, -1.309, 1.571, -1.833, -1.571, 0.0]  # radians

# Home position in Cartesian (for reference, may be in different frame)
HOME_POSITION = {
    'x': -588.63,   # mm
    'y': -133.62,   # mm
    'z': 370.85,    # mm
...
class CartesianRobot:
    ...
    def move(self, x, y, z, max_step=0.005):
        """
        Simplified move function with constrained orientation.

        Always uses the default orientation (end-effector facing down).
        Uses smaller max_step for smoother paths on small movements.

        Args:
            x, y, z: Position in millimeters (relative to base_link)
            max_step: Maximum step size for Cartesian interpolation in meters (default 5mm)
                     Smaller values = smoother but slower planning, better for small movements

        Returns:
            bool: True if movement successful, False otherwise

        Example:
            # Move to position with end-effector always facing down
            robot.move(100, 400, 300)
        """
        self.node.get_logger().info(f"Moving to: x={x}mm, y={y}mm, z={z}mm (constrained orientation)")

        # Use default orientation constraint
        ...
        self.node.get_logger().info(f"Target (meters): x={x_m:.4f}, y={y_m:.4f}, z={z_m:.4f}")
        self.node.get_logger().info(f"Using default orientation: rx={rx:.3f}, ry={ry:.3f}, rz={rz:.3f}")
        ...
        pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w = quat
        req.waypoints = [pose]
        req.max_step = max_step  # Smaller step for smoother paths
        req.jump_threshold = 0.0  # No jump allowed
        req.avoid_collisions = True

        # Plan path using MoveIt
        self.node.get_logger().info(f'Planning Cartesian path (max_step={max_step*1000:.1f}mm)...')
        path_result = self.call_service("/compute_cartesian_path", req)
        ...
        self.node.get_logger().info(f'Executing trajectory...')
    ...
    def go_home(self):
        """
        Move to the home position using joint angles.

        Home position is defined by joint angles [0, -75, 90, -105, -90] degrees.
        This is more reliable than Cartesian movements since it avoids frame confusion
        and collision checking issues.
        ...
        self.node.get_logger().info("Moving to HOME position using joint angles")
        self.node.get_logger().info(f"Joint angles: {HOME_JOINT_ANGLES}")
        self.node.get_logger().info(f"  [0°, -75°, 90°, -105°, -90°, 0°]")
        self.node.get_logger().info("="*60)
        ...
        # Create single waypoint at home position
        point = JointTrajectoryPoint()
        point.positions = HOME_JOINT_ANGLES
        point.time_from_start = Duration(sec=5, nanosec=0)  # 5 seconds to reach home

        trajectory_goal.trajectory.points = [point]

        self.node.get_logger().info('Sending joint trajectory to home position...')

        # Send goal and wait for result
        goal_response = self.call_action(self.trajectory_action_client, trajectory_goal)
        if not goal_response.accepted:
            raise Exception("Home trajectory goal was not accepted by controller")

        self.node.get_logger().info('Trajectory accepted, moving to home...')

        result = self.get_result(self.trajectory_action_client, goal_response)

        if result.error_code == FollowJointTrajectory.Result.SUCCESSFUL:
            self.node.get_logger().info('✓ Reached HOME position successfully!')
            return True
        else:
            self.node.get_logger().error(f'✗ Failed to reach home: {result.error_string} (code {result.error_code})')
            return False
