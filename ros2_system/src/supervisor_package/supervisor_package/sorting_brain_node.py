...
from sort_interfaces.msg import DetectedObjects, WeightEstimate, BoundingBox, SystemStatus
...
class SortingBrainNode(Node):
    ...
    def _init_publishers(self):
        ...
        self.system_status_pub = self.create_publisher(SystemStatus, '/sorting/system_status', 10)
    ...
    def publish_status(self, message: str = None):
        ...
        # Publish SystemStatus message
        self.publish_system_status(message)

        # Also publish placed weight markers
        self.publish_placed_weight_markers()

    def publish_system_status(self, status_message: str = None):
        """Publish detailed system status using SystemStatus message."""
        msg = SystemStatus()
        ...
        # Node identifier
        msg.node_name = "sorting_brain"

        # Current state
        msg.state = self.state.value.upper()

        # Status message
        if status_message:
            msg.status_message = status_message
        else:
            msg.status_message = f"State: {self.state.value}"

        # Progress information
        if self.placed_weights:
            msg.current_step = len(self.placed_weights)
            msg.total_steps = len(self.placed_weights) + len(self.detected_objects)
            if msg.total_steps > 0:
                msg.progress_percent = (msg.current_step / msg.total_steps) * 100.0
        else:
            msg.current_step = 0
            msg.total_steps = 0
        ...
        msg.object_id = self.current_object_id if self.current_object_id is not None else -1
        msg.object_weight = int(self.current_weight) if self.current_weight is not None else -1
        msg.target_bin = ""  # Not used in this system

        # Activity flags
        msg.is_moving = self.state in [SortingState.PICKING, SortingState.PLACING,
                                       SortingState.REARRANGING, SortingState.MOVING_TO_HOME]
        msg.is_gripping = self.holding_object
        msg.is_weighing = self.state == SortingState.WEIGHING
        msg.is_waiting = self.state == SortingState.WAITING_FOR_DETECTION

        # Error/warning information
        msg.has_error = self.state == SortingState.ERROR
        msg.has_warning = False
        msg.error_message = "Error state active" if msg.has_error else ""
