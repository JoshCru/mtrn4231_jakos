...
from sort_interfaces.msg import BoundingBox, SystemStatus
...
class SortingDashboard(Node):
    ...
    def __init__(self):
        ...
        self.sorting_system_status_sub = self.create_subscription(
            SystemStatus, '/sorting/system_status', self.sorting_system_status_callback, 10
        )
        self.motion_system_status_sub = self.create_subscription(
            SystemStatus, '/motion_control/system_status', self.motion_system_status_callback, 10
        )
        ...
        # SystemStatus state variables
        self.sorting_status = None
        self.motion_status = None
        self.last_sorting_activity = ""
        self.last_motion_activity = ""
    ...
    def sorting_system_status_callback(self, msg: SystemStatus):
        """Update sorting brain system status."""
        self.sorting_status = msg
        ...
        if msg.is_gripping:
            activities.append("Gripping")
        if msg.is_weighing:
            activities.append("Weighing")
        if msg.is_waiting:
            activities.append("Waiting")

        self.last_sorting_activity = ", ".join(activities) if activities else "Idle"

    def motion_system_status_callback(self, msg: SystemStatus):
        """Update motion control system status."""
        ...
        self.last_motion_activity = ", ".join(activities) if activities else "Idle"
...
class DashboardGUI:
    ...
    def setup_ui(self):
        ...
        # Activity Status Frame
        activity_frame = ttk.LabelFrame(left_panel, text="Node Activities", padding=10)
        activity_frame.pack(pady=5, fill=tk.X)

        # Sorting Brain Activity
        ttk.Label(activity_frame, text="Sorting Brain:", font=('Arial', 9, 'bold')).pack(anchor=tk.W)
        self.sorting_activity_label = ttk.Label(
            activity_frame,
            text="---",
            font=('Arial', 9),
            foreground='#4CAF50'
        )
        self.sorting_activity_label.pack(anchor=tk.W, padx=10)

        # Motion Control Activity
        ttk.Label(activity_frame, text="Motion Control:", font=('Arial', 9, 'bold')).pack(anchor=tk.W, pady=(5, 0))
        self.motion_activity_label = ttk.Label(
            activity_frame,
            text="---",
            font=('Arial', 9),
            foreground='#4CAF50'
        )
        self.motion_activity_label.pack(anchor=tk.W, padx=10)
        ...
        # Create a simple progress bar using a canvas
    ...
    def update_display(self):
        ...
        self.sorting_activity_label.config(text=self.node.last_sorting_activity or "---")
        self.motion_activity_label.config(text=self.node.last_motion_activity or "---")

        # Update progress bar (prefer motion status if available, otherwise sorting status)
        progress_percent = 0.0
        if self.node.motion_status:
            progress_percent = self.node.motion_status.progress_percent
            self.progress_label.config(
                text=f"{progress_percent:.0f}% (Step {self.node.motion_status.current_step}/{self.node.motion_status.total_steps})"
            )
        elif self.node.sorting_status:
            progress_percent = self.node.sorting_status.progress_percent
            self.progress_label.config(
                text=f"{progress_percent:.0f}% ({self.node.sorting_status.current_step} placed)"
            )
        else:
        ...
        if canvas_width > 1:  # Make sure canvas is initialized
            fill_width = (canvas_width * progress_percent) / 100.0
