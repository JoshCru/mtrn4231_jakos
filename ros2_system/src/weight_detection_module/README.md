...
These calculations are a series of torque calculations ($mass$ x $gravity$ x $radius$) transformed to the coordinate frame of the active joints.
...
```
Joint Torques → Kalman Filter → Torque Deltas → Kinematics → Mass Estimate → Calibration → Output
                                     ↑
                              Baseline Torques
```

1. **Baseline Calibration**: On startup (or when triggered via service), the node collects ~5 seconds of torque samples to establish a baseline with the gripper empty.
2. **Torque Filtering**: Incoming joint torques are smoothed using per-joint Kalman filters.
3. **Mass Estimation**: The difference between current and baseline torques is used with UR5e forward kinematics to compute moment arms, then mass is estimated via τ = m × g × r.
...
```bash
ros2 run weight_detection_module weight_detector_cpp --ros-args -p useSnapping:=true
```

### Visualisation

Use PlotJuggler to monitor the estimated mass in real-time:
...
- **Stationary robot**: Estimates are most accurate when the robot is not moving. Joint accelerations introduce torque noise.
